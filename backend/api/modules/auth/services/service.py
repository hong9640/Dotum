...
from api.modules.user.repositories.user import UserRepository
...
async def check_email_exists(email: EmailStr, db: AsyncSession) -> bool:
    ...
    return await user_repo.exists_by_email(email)

async def create_user(user_data: SignupRequest, db: AsyncSession) -> User:
    """새로운 사용자를 생성하는 서비스 로직"""
    user_repo = UserRepository(db)
    
    is_duplicate = await user_repo.exists_by_email(user_data.username)
    if is_duplicate:
...
async def get_user_by_email(email: EmailStr, db: AsyncSession) -> Optional[User]:
    """이메일로 사용자를 조회 (삭제되지 않은 사용자만)"""
    user_repo = UserRepository(db)
    return await user_repo.get_by_email(email, include_deleted=False)
...
async def login_user(user_data: UserLoginRequest, db: AsyncSession) -> dict:
    ...
    try:
        await token_repo.create(new_refresh_token_entry)
...
async def logout_user(
    ...
    token_repo = RefreshTokenRepository(db)
    deleted = await token_repo.delete_by_hash_and_user_id(hashed_token, user_id)
...
async def refresh_access_token(
    ...
    hashed_refresh_token = hash_token(refresh_token)
    token_repo = RefreshTokenRepository(db)
    token_entry = await token_repo.get_by_hash(hashed_refresh_token)
    ...
    await token_repo.create(new_token_db_entry)
