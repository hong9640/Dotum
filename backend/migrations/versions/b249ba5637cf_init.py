...
def upgrade() -> None:
    ...
    if table_name:
        ...
        if 'ai_model_id' not in columns:
            op.add_column(table_name, sa.Column('ai_model_id', sa.Integer(), nullable=True))
            # 기존 데이터에 대한 기본값 설정 (필요시)
            op.execute(f"UPDATE {table_name} SET ai_model_id = 1 WHERE ai_model_id IS NULL")
            op.alter_column(table_name, 'ai_model_id', nullable=False)
        ...
        if 'ix_train_session_praat_feedback_ai_model_id' not in indexes and 'ix_training_session_praat_feedback_ai_model_id' not in indexes:
            op.create_index(op.f('ix_train_session_praat_feedback_ai_model_id'), table_name, ['ai_model_id'], unique=False)
        
        # model_version 컬럼이 있으면 삭제
        if 'model_version' in columns:
            op.drop_column(table_name, 'model_version')
    else:
