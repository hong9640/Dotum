...
REM =========================
REM  실행 권한 부여(가능한 경우)
REM  - Git Bash/WSL이 있으면 chmod 실행
REM =========================
...
if exist "%REPO%\nul" (
  ...
  echo        아래 명령으로 제거해 주세요 (Git Bash에서 실행 가능):
...
echo  - 훅 경로: %HOOKDIR%
echo.
echo (선택) VS Code를 기본 에디터로:  git config core.editor "code -w"
echo   템플릿 사용 시에는 'git commit' ( -m 없이 ) 를 사용하세요.
echo.
endlocal
